...
drawClockHands quality width height = do
  ...
  let hours = fromIntegral
        (if ctHour time >= 12 then ctHour time - 12 else ctHour time)
...
drawDropShadow = withRadialPattern 0.55 0.55 0.25 0.5 0.5 0.525 $ \pattern ->
  do
...
drawClockFace True = withLinearPattern 0.5 0 0.5 1 $ \pattern -> do
...
drawHourHand quality hours minutes seconds = do
  ...
  rotate ((pi / 6) * hours + (pi / 360) * minutes + (pi / 21600) * seconds)
...
drawMinuteHand quality minutes seconds = do
  ...
  setLineCap LineCapSquare
  setLineJoin LineJoinMiter
  rotate ((pi / 30) * minutes + (pi / 1800) * seconds)
...
runGenerateGTK
  :: GI.Cairo.Context -> Surface -> GenerateCtx -> Generate a -> IO ()
runGenerateGTK ct surface ctx@GenerateCtx {..} doRender =
  renderWithContext ct
    . void
    . flip runReaderT ctx
    . flip runRandT   (pureMT gcSeed)
    $ do
...
go = do
  ...
  geometry <- new
    Gdk.Geometry
    [#minWidth := 32, #minHeight := 32, #maxWidth := 512, #maxHeight := 512]
  #setGeometryHints window
                    (Just window)
                    (Just geometry)
  ...
  on window #buttonPressEvent $ \event -> do
    ...
    case button of
      1 -> do
        ...
        return True
      3 -> do
        #beginResizeDrag window
                         Gdk.WindowEdgeSouthEast
                         3
                         (round x)
                         (round y)
                         time
  ...
  GLib.timeoutAdd GLib.PRIORITY_DEFAULT 1000 (#queueDraw window >> return True)
